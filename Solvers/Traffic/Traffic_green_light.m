...
for i = 1:timesteps
    ...
    if 1
        ...
        fluxU = assign(assign(constField(V0, N), ...
                            assign(constField(1, N), u, '-'), '*'), u, '*');
        [a_j_minus_half, a_j_plus_half] = aspeedTraffic(u, V0);
    else
        % Cubic traffic flow flux function
        fluxU = assign(assign(assign(constField(V0, N),...
        assign(constField(1, N), u, '-'), '*'), u, '*'),...
        assign(constField(1, N), u, '-'), '*');
    ...
    if 0
        ...
        [u, u] = Rusanov(u, u, fluxU, fluxU, cfluxU, cfluxU,...
        a_j_minus_half, a_j_plus_half, 0, 0, dummyRho, dummyRho, dx, dt);	   
    else
